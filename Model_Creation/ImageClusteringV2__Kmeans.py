...
PATH = r'C:\Users\lukez\PycharmProjects\CMSC499\Image Scrape\Images'
...
def create_Model(productImages, number_products, model, product):
    ...
    kmeans = KMeans(n_clusters=number_products, random_state=22)
    kmeans.fit(x)
    ...
    kmeansDictionary = {}
    for group in groups:
        kmeansDictionary[group] = get_cluster_value(groups, group)
    """
    ...
    with open('kmeansDictionary_' + product + '.pkl','wb') as f:
        pickle.dump(kmeansDictionary,f)

    with open('kmeansModel_' + product + '.pkl','wb') as f:
        pickle.dump(kmeans,f)

    with open('pcaModel_' + product + '.pkl','wb') as f:
        pickle.dump(pca,f)
